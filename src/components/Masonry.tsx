...
export default function Masonry({
  ...
  const [grid, containerHeight] = useMemo(() => {
    ...
    const positionedItems = items.map(child => {
      ...
      // Stagger child height. Adjusting the factor slightly for better aspect ratios.
    ...
    return [positionedItems, Math.max(...colHeights)];
  }, [columns, items, width]);
  ...
  return (
    <div ref={containerRef} className="masonry-list" style={{ height: containerHeight, position: 'relative' }}>
      {grid.map(item => {
        return (
          <div
            key={item.id}
