...
function buildItems(pool, seg) {
  // We want to reduce the total density of images in the Dome (fewer duplicates)
  // and create beautiful, uniform gaps by spacing out the coordinates.
  // Instead of every index X (step 2) and 5 rows, we step X by 4.4 and use fewer rows (Y: -2, 0, 2 / -1, 1).
  const xCols = Array.from({ length: Math.ceil(seg / 2.2) }, (_, i) => -37 + i * 4.4);
  const evenYs = [-5, 0, 5];
  const oddYs = [-2.5, 2.5];

  const coords = xCols.flatMap((x, c) => {
    const ys = c % 2 === 0 ? evenYs : oddYs;
    return ys.map(y => ({ x, y, sizeX: 4.0, sizeY: 4.0 }));
  });
  ...
  const normalizedImages = pool.map(image => {
    if (typeof image === 'string') {
      return { src: image, label: 'Fashion Silhouette' };
    }
    return { src: image.src || '', label: image.label || image.alt || 'Fashion Silhouette' };
  });
...
export default function DomeGallery({
  ...
  const rotationRef = useRef({ x: initialTilt, y: 0 });
    ...
    const ro = new ResizeObserver(entries => {
      ...
      if (enlargedOverlay && frameRef.current && mainRef.current) {
        ...
        if (hasCustomSize) {
          ...
          let finalW = tempRect.width;
          let finalH = tempRect.height;
          const maxAllowedW = frameR.width * 0.9;
          const maxAllowedH = frameR.height * 0.85;
  ...
  useGesture(
    {
      ...
      onDrag: ({ event, last, velocity = [0, 0], direction = [0, 0], movement }) => {
        ...
        let nextY = startRotRef.current.y + dxTotal / dragSensitivity;
  ...
  useEffect(() => {
    ...
    const close = () => {
      ...
      animatingOverlay.style.cssText = `position:absolute;left:${overlayRelativeToRoot.left}px;top:${overlayRelativeToRoot.top}px;width:${overlayRelativeToRoot.width}px;height:${overlayRelativeToRoot.height}px;z-index:9999;border-radius: var(--enlarge-radius, 0px);overflow:hidden;box-shadow:0 10px 30px rgba(0,0,0,.35);transition:all ${enlargeTransitionMs}ms ease-out;pointer-events:none;margin:0;transform:none;`;
  ...
  const openItemFromElement = useCallback(
    el => {
      ...
      if (wantsResize) {
        const onFirstEnd = ev => {
          ...
          let finalWidth = tempRect.width;
          let finalHeight = tempRect.height;

          const maxAllowedWidth = frameR.width * 0.9;
          const maxAllowedHeight = frameR.height * 0.85;
    ...
    [enlargeTransitionMs, lockScroll, openedImageHeight, openedImageWidth, segments, stopInertia, unlockScroll]
  );
